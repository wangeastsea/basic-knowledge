...
- 服务端 client-random + service-random + pre-master 生成 master secret
- 客户端 client-random + service-random + pre-master 生成 master secret
- 然后服务端和客户端使用 master secret 加密来传输数据
...
4. clent验证证书有效性，生成pre-master通过服务器公钥加密 发送给server
