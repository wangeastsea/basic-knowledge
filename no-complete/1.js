...
class CheckCommitMsg {
    ...
    async checkCommitMsgHandle() {
        let singlePages = this.getSinglePages()
        // 如果当前分支不用检查提交信息，直接跳过
        ...
        let promptList = [
            {
                type: 'input',
                message: '请输入一个 descriptions:',
                name: 'descriptions',
                default: 'this is a descriptions' // 默认值
            },
            {
                type: 'checkbox',
                message: '请选择需要构建的项目:',
...
let singlePages = getSinglePages()
...
// execSync('git commit -m "--build -d margin -i test"')
// execSync('rm -rf ./.git/.COMMIT_EDITMSG.swp')
// execSync('git commit --amend')

// fs.writeFileSync(
//     './.git/COMMIT_EDITMSG',
//     'git commit -m "--build -d margin -i test"'
// )

// execSync('git log -1 --pretty=%B', async function(err, commitMsg) {
//     await console.log('commitMsg', commitMsg)
// })
